 ...
 */
public class DragonPalaceV2_2 extends Sketch {

    public static void main(String... args) {
        PApplet.main("drawings.DragonPalaceV2_2");
    }
    ...
    public void sketch() {
        for(int iter = 0; iter < 30; iter++) {
            _colours.getColours("constructivist-v2").forEach((name, colour) -> {
                background(colour.black());
                noFill();
                ArrayList<Integer> meshColours = new ArrayList<>();
                meshColours.add(colour.white());
                drawRaggedMeshBackground(meshColours, 1, 5, 255 * 0.2f, 1);
                drawTexture(colour.get(2), 0.9f, 0.8f);
                drawTexture(colour.white(), 0.1f, 0.25f, 2048.0f);
                strokeCap(ROUND);
                println(name);
                squiggleGroup(colour);
                save("dragons-palace-v2_2", name);
            });
        }
    }

    private void squiggleGroup(Colours colour) {
        float strokeWeight = 36f;
        ...
        for (int i = 0; i < strands; i++) {
            ...
            stroke(colour.randAll(), 255 * 0.75f);
            squiggle(
