...
public class FoliageV3 extends Sketch {

    int totalCount = 0;
    float scale = 0.92f;
    int foliageColour = 0xffffff;

    public static void main(String... args) {
        PApplet.main("drawings.FoliageV3");
    }
    ...
    public void sketch() {
        _colours.getColours().forEach((name, colour) -> {
            ...
            drawGrainTexture(c2, 0.85f, _height/2.2f, CUBIC, EASE_OUT);
            noStroke();
            foliageColour = colour.white();
            int lineCount = round(random(1) * 1750) + 750;
            initStems(drawLines, collisionLines, colour, _colours.rand(), 4);
            int drawnCount = 12;
            while (drawnCount < lineCount) {
                float leanLimit = 1.5f;
                float angleRange = PI / 9;
                float minSize = 125;
                drawnCount = drawLines(drawLines, collisionLines, drawnCount, leanLimit, angleRange, minSize);
            }
            save("foliage-v3", lineCount + "-" + name);
        });
    }
    ...
    void initStems(ArrayList<Line> drawLines, ArrayList<Line> collisionLines, Colours foliageColours, Colours stemColours, int stemCount) {
        for (int i = 0; i < stemCount; i++) {
            Vector centre = new Vector(_width / 2 + (random(1) * 3 - 1.5f), _height / 2 + (random(1) * 3 - 1.5f));
            Vector start = new Vector(_width / 2 + (random(1) * 3 - 1.5f), _height / 2 + (random(1) * 3 - 1.5f));
            Line l = new Line(
                    start.getPoint(),
                    300,
                    TAU / (i + 1) + random(1) * TAU / 3,
                    foliageColours,
                    stemColours,
    ...
    class Line {
        ...
        private Point drawTo() {
            float dX = _p.x() + 0.98f * (_q.x() - _p.x());
            float dY = _p.y() + 0.98f * (_q.y() - _p.y());
            return new Point(dX, dY);
        }
