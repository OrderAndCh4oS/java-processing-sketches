...
public class DragonPalaceV7 extends Sketch {

    private ArrayList<ArrayList<Point>> _pointsList;

    public static void main(String... args) {
        PApplet.main("drawings.DragonPalaceV7");
    }
        ...
        for (int iter = 0; iter < 5; iter++) {
            _colours.getColours().forEach((name, colour) -> {
                ...
                for (int j = 0, pointsListSize = _pointsList.size(); j < pointsListSize; j++) {
                    ...
                    popMatrix();
                    drawTexture(colour.black(), 0.4f, 0.5f);
                }
                save("dragons-palace-v7", name);
            });
        }
    }

    private void squiggleGroup() {
        for (int i = 0; i < 64; i++) {
