...
public class PipesV6 extends Sketch {

    public static void main(String... args) {
        PApplet.main("drawings.PipesV6");
    }
    ...
    public void sketch() {
        for (int iter = 0; iter < 10; iter++) {
            _colours.getColours().forEach((name, colour) -> {
                int bg = colour.black();
                background(bg);
                drawTexture(colour.white(), 0.9f, 0.2f, 1024f);
                strokeCap(ROUND);
                fill(colour.rand());
                strokeWeight(0.5f);
