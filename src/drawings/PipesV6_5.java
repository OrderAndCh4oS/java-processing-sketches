...
public class PipesV6_5 extends Sketch {

    public static void main(String... args) {
        PApplet.main("drawings.PipesV6_5");
    }
    ...
    public void sketch() {
        for (int iter = 0; iter < 1; iter++) {
            _colours.getColours().forEach((name, colour) -> {
                int bg = colour.black();
                background(bg);
                drawTexture(colour.randWithWhite(), 0.9f, 0.25f, 1024f);
                strokeCap(ROUND);
                fill(colour.black());
                strokeWeight(0.66f);
                stroke(colour.black(), 0.1f * 255);
                squiggleGroup(colour);
                save("pipes-v6.5", name);
            });
        }
    }
