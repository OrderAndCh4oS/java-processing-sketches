...
public class CliffordAttractorChunkyLinesV3 extends Sketch {

    float a;
    float b;
    float c;
    float d;
    private ArrayList<PGraphics> _sources;

    public static void main(String... args) {
        PApplet.main("drawings.CliffordAttractorChunkyLinesV3");
    }
        ...
        for (int iteration = 0; iteration < 5; iteration++) {
            _colours.getColours().forEach((name, colour) -> {
                ...
                for(int i = 0; i < 8; i++) {
                    ...
                    source.beginDraw();
                    drawBenDayTexture(source, colour.randWithBlack(), 20, random(0.25f, 0.85f));
                    source.endDraw();
                ...
                save("clifford-attractor-chunky-lines-v3", name);
            });
        }
    }
...
//                fill(i % 2 == 0 ? colour.black() : colour.white());
                ...
                mask.endShape(CLOSE);
                mask.endDraw();
                PGraphics source = this._sources.get(new Random().nextInt(this._sources.size()));
                source.mask(mask);
                image(source, 0, 0);
            }
