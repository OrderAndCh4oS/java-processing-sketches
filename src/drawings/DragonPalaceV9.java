...
public class DragonPalaceV9 extends Sketch {

    private ArrayList<ArrayList<Point>> _pointsList;

    public static void main(String... args) {
        PApplet.main("drawings.DragonPalaceV9");
    }
...
//        for (int iter = 0; iter < 5; iter++) {
            _colours.getColours("constructivist-real").forEach((name, colour) -> {
                ...
                for (int k = 0, pointsListSize = _pointsList.size(); k < pointsListSize; k++) {
                    radiusList.add(randomInt(18, 64));
                }
                for (int j = 0, pointsListSize = _pointsList.size(); j < pointsListSize; j++) {
                    drawTexture(colour.black(), 0.3f, 0.5f);
                    pushMatrix();
                    translate(_width / 2, _height / 2);
                    for (int i = _pointsList.get(0).size(); i >= 0; i--) {
                        ArrayList<Point> points = _pointsList.get(j);
                        if (points.size() <= i) continue;
                        Point p = points.get(i);
                        fill(colour.getBlackAndWhiteOnly().get(j % colour.getBlackAndWhiteOnly().size()));
                        stroke(colour.getBlackAndWhiteOnly().get((j + 1) % colour.getBlackAndWhiteOnly().size()), 0.125f * 255);

                        int radius = radiusList.get(j);
                        ellipse(p.x(), p.y(), radius, radius);
                    }
                    popMatrix();
                }
                save("dragons-palace-v9", name);
            });
        }
//    }
