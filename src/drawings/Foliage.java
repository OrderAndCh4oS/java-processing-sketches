...
public class Foliage extends Sketch {
    ...
    public void sketch() {
        _colours.get_colours().forEach((name, colour) -> {
            ...
            while (drawnCount < 15000) {
                ...
                if (random(1) > 0.5) {
                    newLean = random(1) > 0.5 ? Lean.LEFT : Lean.RIGHT;
                }
    ...
    void initStems(ArrayList<Line> drawLines, ArrayList<Line> collisionLines, Colours foliageColours, Colours stemColours, int stemCount) {
        for (int i = 0; i < stemCount; i++) {
            Line l = new Line(new Point(750, 750), 150, random(PI * 2), foliageColours, stemColours, random(1) > 0.5 ? Lean.LEFT : Lean.RIGHT);
            drawLines.add(l);
